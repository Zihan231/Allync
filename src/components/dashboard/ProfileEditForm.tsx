  ...
  function validateTab(tab: ProfileTab): FormErrors {
    ...
    if (tab === "account") {
      ...
      if (form.password && !isValidPasswordLength(form.password)) {
        next.password = pf.accountInfo.passwordError;
      }
    } else if (tab === "social") {
          ...
          {!isEditing ? (
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={isSaving}
                onClick={onCancel}
                className="rounded-full border border-surface-line-strong px-3.5 py-1.5 text-xs font-semibold text-ink-soft transition-colors hover:text-ink disabled:opacity-50"
              >
                {cancelLabel}
              </button>
              <button
                ...
                disabled={isSaving}
                onClick={onSave}
                className="inline-flex items-center gap-1.5 rounded-full bg-accent px-4 py-1.5 text-xs sm:text-sm font-semibold text-bg transition-transform hover:-translate-y-0.5 disabled:opacity-50 shadow-sm"
              >
                {isSaving ? (
