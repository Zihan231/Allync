...
export function ClubNewsFeed({ items, title }: { items: ClubNewsItem[]; title?: string }) {
  const { t } = useLanguage();

  return (
    <div className="rounded-xl border border-surface-line bg-surface/30 p-5">
      <h3 className="font-display text-sm font-bold text-ink">{title ?? t.dashboard.clubOverview.newsFeedTitle}</h3>
